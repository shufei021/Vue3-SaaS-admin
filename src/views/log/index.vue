  ...
  <div class="page-container">
    ...
    <div class="filter-bar">
      <el-form :inline="true" :model="query">
        <el-form-item label="关键词">
          <el-input v-model="query.keyword" placeholder="用户名/操作/模块" clearable style="width: 200px" />
        </el-form-item>
    ...
    <div class="card-base">
      <el-table v-loading="loading" :data="tableData" stripe border style="width: 100%">
        ...
        <el-table-column prop="duration" label="耗时" width="80" align="right">
          <template #default="{ row }">
            <span :style="{ color: row.duration > 300 ? '#f56c6c' : 'inherit' }">{{ row.duration }}ms</span>
          </template>
        </el-table-column>
        <el-table-column prop="createTime" label="操作时间" width="170" />
        <el-table-column label="操作" width="80" fixed="right">
          <template #default="{ row }">
            <el-button link type="primary" size="small" @click="handleDetail(row as AuditLog)">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog v-model="detailVisible" title="日志详情" width="560px">
      <el-descriptions v-if="currentLog" :column="2" border>
        ...
        <el-descriptions-item label="请求 URL" :span="2">{{ currentLog.url }}</el-descriptions-item>
